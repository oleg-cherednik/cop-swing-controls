...
/**
 * A BusyModel implementation allowing to reflet the execution of a Future task. While the job task underlying the
 * Future is running, this model will be set to a <code>busy</code> state.
 * <p>
 ...
 * When you bound a {@link SwingWorker} to this model, this model will be determinate and use the
 * {@link SwingWorker#getProgress()}
 * <p>
 * When you don't need anymore to use this model, you must invoke {@link #dispose()} in order to free all threading
 * resources.<br>
 * 
 * @author Oleg Cherednik
 * @since 09.04.2012
 */
public class FutureBusyModel extends DefaultBusyModel implements PropertyChangeListener {
	private static final long serialVersionUID = -5772369507639501517L;
	/**
	 * Members
	 */
	...
	/**
	 * Reflet a new <code>Future</code> to reflet. This model will be set as <code>undeterminate</code> but
	 * <code>cancellable</code> model.
	 * 
	...
	/**
	 * You can't define the busy state manually on a {@link FutureBusyModel} Use instead the {@link #setFuture(Future)}
	 * for define which task to track.
	 */
	@Override
	...
	/**
	 * Reflet a new <code>Future</code> to reflet. This model will be set as <code>undeterminate</code> and
	 * <code>cancellable</code> if specified.
	 * 
	 * @param future New Future to reflet.
						...
						while (!myFuture.isDone()) {
							...
							try {
								...
								if (myFuture != FutureBusyModel.this.trackedFuture) {
									/**
									 * probably the model must reflet now a different Future We must stop to reflet this
									 * one
									 */
									break;
								}
	...
	public void propertyChange(PropertyChangeEvent evt) {
		synchronized (FutureBusyModel.this) {
			if (evt.getSource() == trackedFuture && (evt.getSource() instanceof SwingWorker)
					&& evt.getPropertyName().equals("progress")) {
